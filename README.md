...
启动或热重载成功后，日志会出现 `[xinghan-gacha-plugin] 已加载 2026-08-02-records-r3` 和应用列表。记录命令命中后会出现 `[xinghan-gacha-plugin/records] 命令已命中`；日志只包含游戏、适配器、私聊状态和错误码，不记录用户 ID、消息内容或任何凭据。发送 `#星瀚抽卡诊断` 可确认当前进程是否已经读取 `HOYO_GACHA_MASTER_KEY`，不会显示密钥内容。
