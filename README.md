...
- 三套现代单图记录页：按卡池分区展示高稀有结果、角色/装备图片、垫抽进度、欧非配色及限定角色 UP/歪标记；
- 可选只读复用本机 `miao-plugin` 与 `ZZZ-Plugin` 的三游戏角色、武器、光锥、音擎和邦布素材，缺失素材使用插件内置图像；
...
记录图由 TRSS-Yunzai 的 Puppeteer 渲染器生成：在同一张图片中按卡池分区，只展开最近 12 个五星或 S 级出货，并展示每次出货的图片、抽数和欧非颜色；蓝紫记录不会进入结果卡。每个卡池仍会分别统计总抽数、平均/最欧/最非出货及当前垫抽；星铁“始发跃迁”按一次性 50 抽首金进度计算，首金后不会显示成可循环的 50 抽保底。限定角色池的 UP/歪标记优先读取记录自带标记，否则用内置常驻角色名单排除判断；缺少名称时显示“待确认”，不冒充确定结果。
...
启动或热重载成功后，日志会出现 `[xinghan-gacha-plugin] 已加载 2026-08-15-records-r6` 和应用列表。记录命令命中后会出现 `[xinghan-gacha-plugin/records] 命令已命中`；日志只包含游戏、适配器、私聊状态、错误码和异常类型，不记录用户 ID、消息内容或任何凭据。发送 `#星瀚抽卡诊断` 可确认当前进程是否已经读取 `HOYO_GACHA_MASTER_KEY`，不会显示密钥内容。
...
pnpm test
pnpm check
pnpm preview:records
pnpm preview:records:serve
pnpm smoke:live -- --help
```

`pnpm preview:records` 会在 `temp/record-previews` 生成三款游戏的本地样例；随后运行 `pnpm preview:records:serve`，可通过 `http://127.0.0.1:4173/` 检查页面。`pnpm check` 不访问网络，也不使用真实账号、Cookie、authkey 或二维码 ticket。真实链接测试只应按[阶段 6 验收清单](docs/live-acceptance.md)在专用测试账号上运行。
