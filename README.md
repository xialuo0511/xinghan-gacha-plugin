...
记录图由 TRSS-Yunzai 的 Puppeteer 渲染器生成：卡池面板固定按左右两列排列，每个面板展示全部五星或 S 级结果；记录较多时会分成多张图片，不折叠也不丢弃。每张图片最多包含 24 个高稀有结果，每批最多发送 8 页；仍有后续时会明确提示使用 `#原神抽卡记录下一批`、`#星铁抽卡记录下一批` 或 `#绝区零抽卡记录下一批` 继续，续发游标在内存中保留 15 分钟。蓝紫记录不会进入结果卡。每个卡池仍会分别统计总抽数、平均/最欧/最非出货及当前垫抽；星铁“始发跃迁”按一次性 50 抽首金进度计算，首金后不会显示成可循环的 50 抽保底。限定角色池的 UP/歪标记优先读取记录自带标记，否则用内置常驻角色名单排除判断；缺少名称时显示“待确认”，不冒充确定结果。原神记录图采用至冬主题的冰蓝、深蓝与银白配色。
