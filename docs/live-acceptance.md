...
- [ ] 原神国服官服：扫码登录、角色发现、Yunzai/miao Cookie 自动同步、`#米游社更新面板` 可用、五池首次同步、再次同步零新增。
- [ ] 扫码成功回复不得出现 `stoken` / `cookie_token`；默认 SQLite 的 `MysUsers` 与 `Users` 关联均已建立，旧 `data/MysCookie` 目录没有新增文件。
- [ ] `#同步米游社Cookie` 可幂等重试；同一账号切换已选 UID 后主 UID 随之更新，现有其他有效 CK 主 UID 不被覆盖，纯手工无 CK 主 UID 会切到扫码角色。
- [ ] 模拟加密文件写入或替换失败后，失败凭据不会留在内存供 miao 同步，且临时文件会清理。
- [ ] 原神同步交互：命令后立即出现开始提示，逐池进度可见，长历史分页页码递增且间隔约 300–450 ms。
- [ ] 原神 B 服：角色发现、选择和抽卡同步。
- [ ] 原神国际服：可信 URL 导入、五池分页、过期链接提示。
- [ ] 星铁国服官服：六池同步，含 `21/22` 联动池独立路由。
- [ ] 星铁渠道服：角色发现、选择和抽卡同步。
- [ ] 星铁国际服：可信 URL 导入、六池分页、`common/gacha_record` 主路径及旧路径回退。
...
- [ ] 记录图：三条差异化命令分别输出原神、星铁、绝区零主题截图，卡池、垫抽、最近记录无截断。
- [ ] 欧非与 UP/歪：欧皇/常态/非酋颜色清晰；限定角色池常驻角色标“歪”、限定角色标“UP”，缺少名称时标“待确认”。
